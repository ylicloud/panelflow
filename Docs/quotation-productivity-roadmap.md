...
### 2026-06-09（报价单三级元素统一维护）

**用户输入**：
- 提出报价单第 2 级"单元属性"维护需求，逐步澄清为三级统一模型：每级 = 用户基础数据 + 系统通用补充。
  - 第 1 级（4 位）：控制柜 + 运费/保费/侧板/备件/附件
  - 第 2 级（8 位）：器件 + 辅料/壳体/安装/包装/唛头/抽真空/干燥剂（现状仅硬编码 5 类）
  - 第 3 级（12 位）：客户元件 + 散热风机/柜内照明等通用元件（现仅 MergeExcel 前端实现、不落库）
- 要求：统一新建表、不影响 PB、计价分离、字典含规格型号字段、时间充裕可三级齐备后交付。

**结论 / 决定**：
- 立新 spec `.kiro/specs/quotation-element-maintenance/`（requirements + design + tasks 已产出）。
- 统一字典表 `STD_ELEMENT_DICT`（Level 区分，含 Xlx/Ggxh/SortOrder/IsDefaultOnImport/IsLocked）。
- 核心原语 `TreeReencodeService`（按位置重编码，等价 PB `报价单重新排序`，新写不继承其 bug）。
- 关键约束：x_lx 绑定 PB 汇总槽位（约 10 槽上限）；器件锁定首位；保留既有计价字段。
- 第 1 级沿用 PB"不区分控制柜与费用项"，列 Phase 3 低优先；汇总报表另立项。

**新增 todo / 修改优先级**：
- Phase 1：字典主数据 + 导入去硬编码（第 2 级字典化）。
- Phase 2：报价单结构维护（第 2/3 级 TreeReencode 写回 BJB）。
